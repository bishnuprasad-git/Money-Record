...
export default class Indivisual_receive extends Component{
    ...
    SaveItem = () => {
        if (this.state.p == 0) {
            ...
            try {
                ...
                u.push().set({ text: this.state.c_name, receive_price: parseFloat(this.state.p).toFixed(2), time:Date.now() })
                ...
                if(found==false)
                {
                    u1.set({total_receivable:-1*parseFloat(this.state.p)})
                    u2.set({text:this.props.route.params.cust_name,amount:-1*parseFloat(this.state.p)})
                }
                else
                {
                    let t=parseFloat(receivable)-parseFloat(this.state.p).toFixed(2)
                    u1.set({total_receivable:t})
                    u2.set({text:this.props.route.params.cust_name,amount:t})
                    if(t==0)
                    {
                        ok=true
                        Alert.alert('Transaction Details' ,`You received Rs.${this.state.p} from ${this.state.c_name} and all transaction between you and ${this.state.c_name} has been cleared...so transaction related data are going to be delete. Press Ok to Go to the home screen`,[{text:'ok',onPress:()=>{
                ...
                if(ok==false)
                {
                Alert.alert('Transaction Details' ,`You received Rs.${this.state.p} from ${this.state.c_name}`,[{text:'ok',onPress:()=>{this.props.navigation.goBack()}}])
                }
